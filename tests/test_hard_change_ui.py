...
def test_oversized_image_is_compressed_before_upload(live_server, page: Page):
    ...
    page.wait_for_function(
        "document.querySelector('input[name=files]').files[0]?.name.endsWith('.webp')")

    uploaded = page.evaluate("""() => {
        const f = document.querySelector('input[name=files]').files[0];
