...
2. **状态图**：进入单板查看线性的节点演进（最新在上，工作区置顶，每节点带改动摘要）；点节点看完整 BOM 与 diff 高亮，「下载 CSV」可二选一导出：**完整 BOM**（两列 `Reference,Part`，不贴位号不导出）或**本节点修改项**（三列 `Reference,Part,OP`，即本节点相对上一个节点的改动，可再作为修改清单导入到别处重放）。
