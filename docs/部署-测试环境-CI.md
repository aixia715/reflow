...
**前提：tag 指向的提交必须已经在 `master` 分支历史里。** 流水线第一个 job
`verify-tag-on-master` 会用 `git merge-base --is-ancestor` 校验这一点，tag 打在
还没合并的分支/PR 提交上会直接报错终止，不会构建也不会发布镜像。

发版方式（先合并到 master，再对 master 上的提交打 tag）：
